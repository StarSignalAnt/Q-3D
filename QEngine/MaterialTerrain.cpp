...
MaterialTerrain::MaterialTerrain() {
    ...
    SetVertexShader("Engine/Shader/Terrain/mat_terrain.vsh");
    SetPixelShader("Engine/Shader/Terrain/mat_terrain.psh");
    ...
    PSOCreateInfo.GraphicsPipeline.RTVFormats[0] = QEngine::m_pSwapChain->GetDesc().ColorBufferFormat;
    // Set depth buffer format which is the format of the swap chain's back buffer
    PSOCreateInfo.GraphicsPipeline.DSVFormat = QEngine::m_pSwapChain->GetDesc().DepthBufferFormat;
    ...
    QEngine::m_pDevice->CreateGraphicsPipelineState(PSOCreateInfo, &m_Pipeline);
    ...
    PSOCreateInfo.PSODesc.Name = "TerrainAdd PSO";
    ...
    QEngine::m_pDevice->CreateGraphicsPipelineState(PSOCreateInfo, &m_PipelineAdd);
    ...
    m_PipelineAdd->GetStaticVariableByName(SHADER_TYPE_PIXEL, "LightingConstants")->Set(m_LightingBuffer);
...
void MaterialTerrain::Bind(bool add) {
    ...
    if (add) {
        m_SRBAdd->GetVariableByName(SHADER_TYPE_PIXEL, "v_Texture")->Set(col->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
        m_SRBAdd->GetVariableByName(SHADER_TYPE_PIXEL, "v_TextureNorm")->Set(norm->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
        m_SRBAdd->GetVariableByName(SHADER_TYPE_PIXEL, "v_TextureSpec")->Set(spec->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
        m_SRBAdd->GetVariableByName(SHADER_TYPE_PIXEL, "v_TextureLayer")->Set(m_Textures[3]->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
        ...
        {
            MapHelper<TerrainConstants> dd(QEngine::m_pImmediateContext, m_UniformBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
        ...
        {
            MapHelper<LightingConstants> dd(QEngine::m_pImmediateContext, m_LightingBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
            ...
            dd[0].v_LightRange = glm::vec4(m_Light->GetComponent<LightComponent>()->GetRange(), 0, 0, 0);
            dd[0].v_Layers = int4(m_Indices[0], 0, 0, 0);
        }
        //map_data.Unmap();
