...
struct ActorConstant {
    ...
    glm::mat4 g_Bones[100];
};

MaterialSkeletal::MaterialSkeletal() {
    ...
    SetVertexShader("Engine/Shader/Skeletal/skeletal.vsh");
    SetPixelShader("Engine/Shader/Skeletal/skeletal.psh");

    m_UniformBuffer = CreateUniform(sizeof(ActorConstant), "Actor Uniform Buffer - MVP");
    ...
    LayoutElement indices;
    LayoutElement weights;
    ...
    indices.InputIndex = 5;
    indices.NumComponents = 4;
    indices.ValueType = VALUE_TYPE::VT_FLOAT32;
    indices.IsNormalized = false;


    weights.InputIndex = 5;
    weights.NumComponents = 4;
    weights.ValueType = VALUE_TYPE::VT_FLOAT32;
    weights.IsNormalized = false;
...
//    RasterizerStateDesc r_desc;
 ...
 //   GraphicsPipelineStateCreateInfo gp_desc;
    ...
    Q3D::Engine::QEngine::GetDevice()->CreateGraphicsPipelineState(gp_desc, &ps2);
...
void MaterialSkeletal::Bind(bool add) {
    ...
    if (add) {
        ...
        {
            MapHelper<ActorConstant> map_data(Q3D::Engine::QEngine::GetContext(), m_UniformBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
           ...
           // float4x4 model = Engine::m_Node->GetWorldMatrix();
            ...
            for (int i = 0;i < 100;i++) {
                map_data[0].g_Bones[i] =m_Bones[i];
            }
        }
