...
struct SkyConstant {
    ...
    glm::vec4 g_AtmosRadius;
    glm::vec4 g_LightDir;
    glm::vec4 g_PlanetRadius;
    glm::vec4 g_RayLeigh;
    glm::vec4 g_Mie;
    glm::vec4 g_SunIntense;
    glm::vec4 g_DayFactor;
    glm::vec4 g_Time;
...
MaterialSky::MaterialSky() {
    ...
    SetVertexShader("Engine/Shader/MaterialSky/sky.vsh");
    SetPixelShader("Engine/Shader/MaterialSky/sky.psh");

    m_UniformBuffer = CreateUniform(sizeof(SkyConstant), "Basic Uniform Buffer - MVP");
...
void MaterialSky::Bind(bool add) {
  ...
  //  m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "v_Texture")->Set(m_Textures[0]->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
    ...
    {
        MapHelper<SkyConstant> map_data(Q3D::Engine::QEngine::GetContext(), m_UniformBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
       ...
       // float4x4 model = Engine::m_Node->GetWorldMatrix();
        ...
        map_data[0].g_CameraPos = glm::vec4(m_CameraPos, 0);
        map_data[0].g_LightDir = glm::vec4(m_SunDir, 0);
        map_data[0].g_RayLeigh = glm::vec4(m_RayLeigh, 0);
        map_data[0].g_AtmosRadius = glm::vec4(m_AtmosRadius, 0, 0, 0);
        map_data[0].g_PlanetRadius = glm::vec4(m_PlanetRadius, 0, 0, 0);
        map_data[0].g_Mie = glm::vec4(m_Mie, 0, 0, 0);
        map_data[0].g_SunIntense = glm::vec4(m_SunIntense, 0, 0, 0);
        map_data[0].g_DayFactor = glm::vec4(m_DayLight, 0, 0, 0);
        map_data[0].g_Time = glm::vec4(m_Time, 0, 0, 0); // <-- Add this
    ...
    //Vivid::m_pI
    // mmediateContext->SetPipelineState(m_Pipeline);


}

void MaterialSky::Render() {
