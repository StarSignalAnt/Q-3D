...
struct DepthConstant {
    ...
    glm::vec4 g_CameraExt;// .x = effective range, .yzw unused
    glm::vec4 g_ToneParams;
    glm::vec4 g_AmbientColor;
    glm::mat4 bones[100];
};

MaterialSkeletalDepth::MaterialSkeletalDepth() {


    //m_BRDF = new Texture2D("Engine/Maps/BRDF.png");


    SetVertexShader("Engine/Shader/SkeletalDepth/skeletaldepth.vsh");
    SetPixelShader("Engine/Shader/skeletalDepth/skeletaldepth.psh");

    m_UniformBuffer = CreateUniform(sizeof(DepthConstant), "Basic Uniform Buffer - MVP");
...
void MaterialSkeletalDepth::Bind(bool add) {
    ...
    {
        MapHelper<DepthConstant> map_data(Q3D::Engine::QEngine::GetContext(), m_UniformBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
       ...
       //    map_data[0].g_LightColor = glm::vec4(lc->GetColor(), 1.0f); // White light color
        ...
        map_data[0].g_CameraExt = glm::vec4(m_CameraExt.x, m_CameraExt.y, 0, 0);
...
void MaterialSkeletalDepth::Render() {
