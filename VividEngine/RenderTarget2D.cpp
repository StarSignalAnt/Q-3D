...
RenderTarget2D::RenderTarget2D(int width, int height,bool isdepth) {
	...
	if (isdepth) {
		...
		desc.Format = Vivid::m_pSwapChain->GetDesc().ColorBufferFormat;
	}
	...
	Vivid::m_pDevice->CreateTexture(desc, nullptr, &m_Texture);
	...
	Vivid::m_pDevice->CreateTexture(depth, nullptr, &m_DepthTexture);
...
void RenderTarget2D::ClearZ() {
	...
	Vivid::m_pImmediateContext->ClearDepthStencil(m_DepthView, CLEAR_DEPTH_FLAG, 1.0f, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
...
//	}
	...
	Vivid::SetBoundRT2D(this);

	Vivid::m_pImmediateContext->SetRenderTargets(1, &m_RTView, m_DepthView, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
	Vivid::m_pImmediateContext->ClearRenderTarget(m_RTView, ClearColor, RESOURCE_STATE_TRANSITION_MODE_VERIFY);
	Vivid::m_pImmediateContext->ClearDepthStencil(m_DepthView, CLEAR_DEPTH_FLAG, 1.0f, 0, RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
...
void RenderTarget2D::Release() {
	...
	auto pRTV = Vivid::m_pSwapChain->GetCurrentBackBufferRTV();

	Vivid::m_pImmediateContext->SetRenderTargets(1, &pRTV, Vivid::m_pSwapChain->GetDepthBufferDSV(), RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
	Vivid::SetBoundRT2D(nullptr);
