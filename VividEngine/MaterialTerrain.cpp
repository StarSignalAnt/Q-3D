...
MaterialTerrain::MaterialTerrain() {
    ...
    PSOCreateInfo.GraphicsPipeline.RTVFormats[0] = Vivid::m_pSwapChain->GetDesc().ColorBufferFormat;
    // Set depth buffer format which is the format of the swap chain's back buffer
    PSOCreateInfo.GraphicsPipeline.DSVFormat = Vivid::m_pSwapChain->GetDesc().DepthBufferFormat;
    ...
    Vivid::m_pDevice->CreateGraphicsPipelineState(PSOCreateInfo, &m_Pipeline);
    ...
    Vivid::m_pDevice->CreateGraphicsPipelineState(PSOCreateInfo, &m_PipelineAdd);
...
void MaterialTerrain::Bind(bool add) {
    ...
    if (add) {
        ...
        {
            MapHelper<TerrainConstants> dd(Vivid::m_pImmediateContext, m_UniformBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
        ...
        {
            MapHelper<LightingConstants> dd(Vivid::m_pImmediateContext, m_LightingBuffer, MAP_WRITE, MAP_FLAG_DISCARD);
