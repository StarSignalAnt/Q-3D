...
TerrainMeshBuffer::TerrainMeshBuffer(TerrainMesh* mesh) {
	...
	Vivid::m_pDevice->CreateBuffer(vbuf, &vdata, &VertexBuffer);
	...
	Vivid::m_pDevice->CreateBuffer(ibuf, &idata, &IndexBuffer);
	m_Mesh = mesh;
}
...
void TerrainMeshBuffer::Update() {
	...
	Vivid::m_pImmediateContext->UpdateBuffer(VertexBuffer, 0, VertexBuffer->GetDesc().Size, m_Mesh->GetVertices().data(), RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
