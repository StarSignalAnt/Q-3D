...
glm::mat4 DiligentToGLM2(const Diligent::float4x4& diligentMatrix)
{
...
void SkeletalDepthRendererComponent::OnRenderDepth(GraphNode* camera) {
	...
	for (auto& sub : mesh->GetSubMeshes()) {
		...
		for (int i = 0;i < 100;i++) {
			...
			mat->SetBone(DiligentToGLM2(bones[i]), i);
