...
void main(in VSInput VSIn, out PSInput PSIn)
{
    ...
    float height = v_TextureHeight.SampleLevel(v_TextureHeight_sampler, VSIn.Uv.xy, 0).r*0.05;
    float3 displacedPos = VSIn.Pos + VSIn.Norm * height;

    float3 worldPos = mul(float4(displacedPos, 1.0), g_ModelMatrix).xyz;
    ...
    PSIn.Pos = mul(float4(displacedPos, 1.0), g_MVPMatrix);
    PSIn.Uv = VSIn.Uv;
